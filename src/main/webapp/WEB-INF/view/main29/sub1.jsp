...
<c:if test="${not empty message}">
    <div style="background-color: pink; padding: 20px;">${message}</div>
</c:if>
<h3>고객 조회</h3>
<form>
    고객 번호
...
<c:if test="${not empty customer}">
    ...
    <form action="/main29/sub1/delete" method="post" onsubmit="confirm('삭제하시겠습니까?')">
        <div style="display: none">
            <input type="text" name="id" value="${customer.id}">
        </div>
        <div>
            <button style="background-color: skyblue">삭제</button>
        </div>
    </form>
</c:if>
